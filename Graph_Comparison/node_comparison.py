...
def main():
    ...
    for benign_graphs in benign_for_comparison:
        compare_node_features(*benign_graphs)
              

    for malicious_graphs in malicious_for_comparison:
        compare_node_features(*malicious_graphs)

    for graphs in counter_graphs_for_comparison:
        compare_node_features(*graphs)
...
def compare_node_features(g1_data, g2_data):
    g1, g1_name = g1_data
    g2, g2_name = g2_data

    print(f"Comparing {g1_name} and {g2_name} with node features")
    ...
    for ntype in g1.ntypes:
        if ntype in g2.ntypes:
            ...
            print(f"Feature similarity for node type '{ntype}': {similarity.item():.4f}")
    print()
